...
export default function CVTool() {
  ...
  const submitAnalysis = async () => {
    ...
    try {
      ...
      // Use mock API
      const data = await mockAPI.cv.analyzeStructured(form)

      setResponse(data)
      setIsSubmitting(false)
      setStep(3)
    } catch (e: any) {
      const msg = e?.message || 'Failed to analyze CV. Please try again.'
      setError(msg)
      setIsSubmitting(false)
      setStep(1)
    }
  ...
  const applySuggestion = async (suggestion: any, index: number) => {
    ...
    try {
      // Use mock API
      const data = await mockAPI.cv.applySuggestion({
